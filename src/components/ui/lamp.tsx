...
/**
 * Top-down "lamp" beam used sparingly to spotlight a section heading.
 * Adapted from the Aceternity lamp effect; tuned to the night-blue palette.
 */
export const LampContainer = ({
  ...
  const animateWide = reduce ? {} : { opacity: 1, width: "30rem" };
  ...
  return (
    ...
    >
      <div className="relative flex w-full flex-1 scale-y-125 items-center justify-center isolate z-0">
        ...
        <motion.div
          initial={{ width: "8rem" }}
          whileInView={reduce ? {} : { width: "16rem" }}
          viewport={{ once: true }}
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
          className="absolute inset-auto z-30 h-36 w-64 -translate-y-[6rem] rounded-full bg-accent/70 blur-2xl"
        />
        <motion.div
          initial={{ width: "15rem" }}
          whileInView={reduce ? {} : { width: "30rem" }}
          viewport={{ once: true }}
          transition={{ delay: 0.3, duration: 0.8, ease: "easeInOut" }}
      ...
      <div className="relative z-50 flex -translate-y-[8.5rem] flex-col items-center px-5 md:-translate-y-80">
        {children}
      </div>
    </div>
  );
